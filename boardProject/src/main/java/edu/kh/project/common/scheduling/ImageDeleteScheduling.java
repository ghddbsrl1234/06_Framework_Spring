...
@Component // IOC 관련 : 특별한 역할 없이 단순 Bean 등록
@Slf4j
...
public class ImageDeleteScheduling {
	...
	*/
	...
	// @Scheduled(cron = "0,30 * * * * *") => 시계 초 단위가 0, 30이 될 때 마다 수행
	// @Scheduled(cron = "0 0 * * * * ") => 매 시간 마다 수행
	// @Scheduled(cron = "0 0 0 * * * ") => 자정
	// @Scheduled(cron = "0 0 12 * * * ") => 정오
	// @Scheduled(cron = "0 0 0 1 * * ") => 매 달 1일
	...
	public void scheduling() {
		...
		// 두 배열을 하나로 합침
		// imageArr이라는 빈 배열을 boardArr과 memberArr의 길이 만큼의 크기로 만들기
		File[] imageArr = new File[boardArr.length + memberArr.length];
		
		// 배열 내용 복사 (깊은 복사)
		// System.arraycopy(복사할배열, 복사시작인덱스번호, 새로운배열, 새로운배열복사시작번호, 복사끝번호)
		...
		if(!serverImageList.isEmpty()) {
			...
			for(File serverImage : serverImageList) {
				...
				if(dbImageList.indexOf( serverImage.getName() ) == -1) {
					...
					log.info(serverImage.getName() +  " 가 삭제되었습니다.");
				}
