...
// 브라우저에서는 기본적으로 다른 출처(도메인, 프로토코르 포트 등)의 요청을 차단함.
// -> 클라이언트와 서버의 출처가 다를 때는 CORS 설정을 적절히 해주어야
//	  정상적인 http 통신을 할 수 있다.

@CrossOrigin(origins = "http://localhost:5173") // http://1ocalhost:5173 자원을 공유하는 사이다.
@RestController
public class myAppController {
	...
	public String getUserInfo(@RequestBody Map<String, Object> map) {
		// message 리턴
		// 만약에 요청 데이터 중 name값이 홍길동이고, age 값이 20이면
		// "홍길동님은 20세 입니다" 리턴
		// 같지 않다면 "데이터가 없습니다" 리턴
